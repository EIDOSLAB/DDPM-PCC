...
def plot(pointcloud,name_file):

    if len(pointcloud.shape) == 3:
        assert pointcloud.shape[0] == 1
        pointcloud = pointcloud.squeeze(0)
    
    x_coords = pointcloud[:, 0].numpy()
    y_coords = pointcloud[:, 1].numpy()
    z_coords = pointcloud[:, 2].numpy()

    fig, axs = plt.subplots(1, 3, figsize=(15, 5))

    axs[0].hist(x_coords, bins=30, color='r', alpha=0.7)
    axs[0].set_title('Coordinate distribution X')

    axs[1].hist(y_coords, bins=30, color='g', alpha=0.7)
    axs[1].set_title('Coordinate distribution Y')

    axs[2].hist(z_coords, bins=30, color='b', alpha=0.7)
    axs[2].set_title('Coordinate distribution Z')

    plt.savefig(name_file, dpi=200, bbox_inches='tight', pad_inches=0.01)
    plt.close()
...
class ShapeNetCore(Dataset):
    ...
    def load(self):
        ...
        with h5py.File(self.path, mode='r') as f:
            for i, (pc, pc_id, cate_name) in enumerate(_enumerate_pointclouds(f)):
                ...
                elif self.scale_mode == 'global_unit':
                    shift = pc.mean(dim=0).reshape(1, 3)
